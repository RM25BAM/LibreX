...
import React, {
  ...
  type FSProperty = {
    title?: string;
    address?: string;
    priceOrRent?: number; // monthly rent
    imageUrls?: string[];
    type?: "sale" | "rent";
    ...
    lng?: number;
    ownerUid?: string;
    ownerPhone?: string; // optional (fallback to users/{uid}.phone)
  };
  ...
  type UIProperty = {
    ...
    rentNumeric: number; // monthly rent (for deposit math)
    image: string;
    images: string[];
    bed: number;
    bath: number;
  ...
  }> = ({ onToggleMap, showMap, searchValue, onSearchChange }) => (
  ...
  const ListingCard: React.FC<{
  ...
  }> = ({ id, title, location, price, image, bed, bath, variant = "row", onClick }) => {
    ...
    return (
      <div
        ...
        className={
          isGrid
            ? "group bg-white rounded-2xl border border-slate-200 overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all duration-300"
            : "group grid grid-cols-[160px,1fr] bg-white rounded-2xl border border-slate-200 overflow-hidden cursor-pointer shadow-sm hover:shadow-md transition-all duration-300"
        }
      >
  ...
  const MapErrorBanner: React.FC<{ error: unknown; apiKey?: string }> = ({ error, apiKey }) => {
        ...
        <div className="max-w-xl">
          ...
          {!apiKey && (
            <p className="mt-4 text-slate-600 text-sm">
              Missing API key. Set <code>VITE_APP_Maps_API_KEY</code>.
            </p>
          )}
        </div>
      </div>
  ...
  /** ---------------- Detail Sheet (Phone + Deposit) ---------------- */
  const DetailPanel: React.FC<{
    property: UIProperty | null;
    onClose: () => void;
    onPayDeposit: (amount: number) => Promise<void>;
    paying: boolean;
    error: string | null;
  }> = ({ property, onClose, onPayDeposit, paying, error }) => {
    const [idx, setIdx] = useState(0);
    const images = property?.images?.length ? property.images : property?.image ? [property.image] : [];
    const depositAmount = (property?.rentNumeric ?? 0) * 2; // 1 month rent × 2 (first + security)
    ...
    const telHref = property?.ownerPhone ? `tel:${property.ownerPhone}` : undefined;
            ...
            >
              ...
              <div className="p-4 sm:p-6 space-y-5 overflow-y-auto h-[78vh] lg:h-full">
                        ...
                        {images.map((_, i) => (
                          <button
                            ...
                            className={`h-2.5 w-2.5 rounded-full ${
                              i === idx ? "bg-white shadow ring-1 ring-slate-300" : "bg-white/60"
                            }`}
                  ...
                  <div className="flex gap-2">
                    <a
                      ...
                      className={`inline-flex items-center gap-2 rounded-xl ${
                        telHref ? "bg-emerald-600 hover:bg-emerald-500" : "bg-slate-300 cursor-not-allowed"
                      } text-white px-4 py-2.5 shadow-sm`}
                      aria-disabled={!telHref}
                    >
                      <FaPhone />
                      {telHref ? "Call owner" : "Phone not available"}
                    </a>
                  </div>
                  {!telHref && (
                    <div className="text-xs text-slate-500">
                      The owner didn’t add a phone number for this listing.
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-2 pb-30">
                  <p className="text-sm text-slate-700 font-medium">Pay deposit</p>
                  <div className="flex items-center gap-2">
                    <input
                      type="text"
                      readOnly
                      value={property?.rentNumeric ? `$${(depositAmount).toLocaleString("en-US")}` : ""}
                      className="w-full rounded-xl border border-slate-300 bg-slate-50 px-3 py-2 shadow-sm text-slate-700"
                    />
                    <button
                      disabled={paying || !property?.rentNumeric}
                      onClick={() => onPayDeposit(depositAmount)}
  ...
  const RentListing: React.FC = () => {
      ...
      const load = async () => {
        try {
          ...
          const items: UIProperty[] = snap.docs.map((docSnap) => {
            ...
            const rent = typeof d.priceOrRent === "number" ? d.priceOrRent : 0;
            const imgs = Array.isArray(d.imageUrls) ? d.imageUrls.filter(Boolean) : [];
            const bed = toInt(d.bed, 0);
            const bath = toInt(d.bath, 0);
  
            return {
              id: docSnap.id,
              title: d.title || "Rental Home",
              location: d.address || "—",
              price: rent.toLocaleString("en-US"),
              rentNumeric: rent,
    ...
    // Lazy fetch phone from users/{uid} if not on listing
    const ensurePhoneOnSelected = React.useCallback(async (prop: UIProperty) => {
      if (prop.ownerPhone || !prop.ownerUid) return;
      try {
        const userDoc = await getDoc(doc(db, "users", prop.ownerUid));
        const phone = (userDoc.exists() ? (userDoc.data() as any)?.phone : null) ?? null;
        if (phone) {
          setProperties((prev) => prev.map((p) => (p.id === prop.id ? { ...p, ownerPhone: phone } : p)));
        }
      } catch (err) {
    ...
    const handlePropertyClick = async (id: string) => {
      ...
      if (typeof prop.lat !== "number" || typeof prop.lng !== "number") {
        try {
          ...
          if (coords) {
            setProperties((prev) =>
              prev.map((p) => (p.id === id ? { ...p, lat: coords.lat, lng: coords.lng } : p))
            );
            setTimeout(() => {
    ...
    // Pay deposit -> writes to rentals/{rentalId}
    const payDeposit = async (amount: number) => {
      try {
        setPayError(null);
        if (!selected) return;
        if (!amount || amount <= 0) {
          setPayError("Invalid deposit amount.");
          return;
        }
        ...
        const tenantUid = user.uid;
        const landlordUid = selected.ownerUid;
        if (!landlordUid) {
          setPayError("Listing is missing owner information.");
          return;
        }
  
        setPaying(true);
  
        // Firestore will auto-create the "rentals" collection on this first write if it doesn't exist.
        await addDoc(collection(db, "rentals"), {
          propertyId: selected.id,
          landlordUid,
          tenantUid,
          deposit: Number(amount), // 2 × monthly rent (hard-coded calc)
          currency: "USD",
          state: "DEPOSIT_SENT",   // later: DEPOSIT_ACCEPTED | DEPOSIT_REJECTED
          escrowAddress: null,     // filled after landlord accepts & escrow is deployed
        ...
        console.error(e);
        setPaying(false);
        setPayError(e?.message || "Failed to record deposit.");
      }
    };
    ...
    return (
      <div className="h-screen flex flex-col">
        ...
        >
          {/* Map column */}
          <div
            className={`hidden lg:block h-full min-h-[320px] relative ${
          ...
          <div className="h-full overflow-y-auto p-4 sm:p-6 bg-gradient-to-b from-white to-slate-50">
            <div className="flex items-baseline justify-between mb-4">
              <h2 className="text-2xl font-bold tracking-tight text-slate-900">
                {loading ? "Loading…" : error ? "Couldn't load" : `${filtered.length} Places for Rent`}
              </h2>
              <div className="flex items-center gap-2">
                <button
                  onClick={toggleCollapse}
                  className="lg:hidden rounded-lg border border-slate-200 bg-white px-3 py-1.5 text-sm shadow-sm"
                >
                  {isCollapsed ? "Show map" : "Hide map"}
                </button>
